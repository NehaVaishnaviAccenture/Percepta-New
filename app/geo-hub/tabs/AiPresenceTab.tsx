...
export default function AiPresenceTab({ result, resultComps }: TabProps) {
  ...
  const signals = [
    { key: 'vis',  label: 'Visibility',  weight: 30, q: 'How often your brand appears in AI answers.',                                        score: vis,  tier: visTier,  rankVal: rank(vis, allVis),   avgVal: avg(allVis),  total: allVis.length },
    { key: 'sent', label: 'Sentiment',   weight: 20, q: 'Whether AI talks about your brand in a positive, neutral, or negative way.',        score: sent, tier: sentTier, rankVal: rank(sent, allSent), avgVal: avg(allSent), total: allSent.length },
    { key: 'prom', label: 'Prominence',  weight: 20, q: 'Where your brand appears in the answer, the higher up the more prominent.',        score: prom, tier: promTier, rankVal: rank(prom, allProm), avgVal: avg(allProm), total: allProm.length },
  ];
